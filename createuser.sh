...
do
    if [[ -z "$pp" ]]
    then
        echo -e "${Red}Password prefix cannot be empty, Try again${Color_Off}"
    fi
    if [[ "$pp" == *" "* ]]
    then
...
while [[ !( -z "$nou" ) ]] && [[ !("$nou" =~ ^[0-9]+$) ]]
do
    echo -e "${Red}Invalid value, Try again${Color_Off}"
    printf "${White}Number of users : ${Color_Off}"
    read nou
done
...
do
    ...
    printf "${Cyan}Username : $username${Color_Off}\t${Yellow}Password : $Password${Color_Off}\n"
